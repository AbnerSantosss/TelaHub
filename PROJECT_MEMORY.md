...
- **[Otimização de Performance e Polling Inteligente - Maio/2026]**:
  ...
  - **Ambiente de Testes Isolado (Staging)**: Criação da branch `production` e alteração das portas de contêineres no `docker-compose.yml` (`frontend` para `8096`, `backend` para `3006`, `pgadmin` para `5051`) para permitir implantação concomitante via Portainer e testes através do Cloudflare Tunnel sem afetar as portas padrão (`8095`, `3005`, `5050`) do ambiente principal.
