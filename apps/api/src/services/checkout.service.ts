...
import { estimateMonthlyCents, type BillingInterval } from './subscription.service';
import type {
  CheckoutEventType,
  CheckoutStatus,
...
export interface PublicCheckoutSession {
  ...
  screens: number;
  /** `monthly` | `yearly` — o que o seletor do checkout escolheu. */
  billingInterval: BillingInterval;
  /** Valor MENSAL EQUIVALENTE, inclusive quando `billingInterval` é `yearly`. */
  amountCents: number;
  amount: number;
  currency: 'BRL';
  ...
  /** Consentimento de novidades, para a tela refletir a escolha ao retomar a sessão. */
...
/**
 * Normaliza a coluna `billingInterval` (texto no banco) para o tipo fechado.
 *
 * Qualquer valor inesperado vira `monthly`: sessões anteriores à migração e
 * dado torto no banco caem no preço de TABELA, nunca no descontado — o erro
 * barato é cobrar o cheio e o cliente reclamar, não faturar o anual sem
 * compromisso anual nenhum.
 */
...
function toPublic(session: CheckoutSession): PublicCheckoutSession {
  return {
    ...
    billingInterval: normalizeInterval(session.billingInterval),
    ...
    // Devolvido para a tela poder REFLETIR a escolha de quem retoma a sessão
    // por link. Sem isto a caixa voltaria limpa e o PATCH seguinte gravaria
    // `false` — uma revogação que a pessoa não pediu.
...
export interface CheckoutMetrics {
  ...
  amounts: {
    currency: 'BRL';
    // ⚠️ DOIS NÚMEROS DIFERENTES, e confundi-los é o defeito US-A-05 visto do
    // lado do relatório. `*Cents` é o MENSAL EQUIVALENTE (comparável entre uma
    // venda mensal e uma anual, é o que responde "quanto de MRR entrou").
    // `*CashCents` é o CAIXA (o que efetivamente foi cobrado; no anual, os 12
    // meses de uma vez). Um total só obrigaria o painel a escolher entre
    // subestimar o caixa em 12× ou inflar o MRR em 12× — as duas versões
    // erradas, e nenhuma delas com aviso.
    /** Convertido, em MENSAL EQUIVALENTE. */
    paidCents: number;
    /** Convertido, em CAIXA do ciclo. */
    paidCashCents: number;
    /** Oportunidade perdida: valor parado em sessões abandonadas (mensal equivalente). */
    abandonedCents: number;
    abandonedCashCents: number;
    /** Valor em sessões aguardando pagamento (ainda recuperável). */
    pendingCents: number;
    pendingCashCents: number;
  };
  /** Vendas pagas separadas por periodicidade — o mix mensal × anual. */
  paidByInterval: Record<
    BillingInterval,
    { sessions: number; monthlyEquivalentCents: number; cashCents: number }
  >;
...
export class CheckoutService {
  /**
   * Valor MENSAL EQUIVALENTE da sessão, em centavos, SEMPRE recalculado no
   * servidor a partir do catálogo. Mesma fórmula de
   * `subscriptionService.estimateMonthlyCents` (`max(telas, minScreens) × preço
   * unitário do intervalo`, `0` para plano grátis ou sob consulta) — se o
   * checkout usasse outra conta, prometeria um preço que a fatura não cumpre.
   *
   * A fórmula NÃO muda no anual: o que muda é só o preço unitário que entra
   * nela. Guardar o mensal-equivalente (e não o total do ano) é o que mantém
   * `amountCents` comparável entre sessões mensais e anuais no relatório de
   * abandono — o total do ano sai multiplicando por 12.
   */
  async computeAmountCents(
    ...
    return estimateMonthlyCents(plan, screens, interval);
  }
  ...
  async createSession(
    ...
    const amountCents = await this.computeAmountCents(input.planCode ?? null, screens, interval);
      ...
      data: {
        ...
        events: {
          create: events.map((type) => ({
            ...
            metadata:
              type === 'plan_selected'
                ? JSON.stringify({ planCode: input.planCode, screens, interval })
                : null,
          })),
        },
      },
  ...
  async updateSession(
    ...
    const currentInterval = normalizeInterval(session.billingInterval);
    const nextInterval = input.interval ? normalizeInterval(input.interval) : currentInterval;
    ...
    const intervalChanged = nextInterval !== currentInterval;

    // Recalcula sempre: qualquer PATCH é um passo novo e congela o preço atual
    // do catálogo para esta sessão.
    const amountCents = await this.computeAmountCents(nextPlanCode, nextScreens, nextInterval);
    ...
    // Virar a chavinha do anual é escolher OUTRO preço, então também registra
    // `plan_selected`. Não se cria tipo de evento novo de propósito: o funil
    // conta plano selecionado por `planCode` preenchido, não por evento, então
    // isto não infla taxa nenhuma — só deixa o rastro de quando a pessoa mudou
    // de intervalo, que é o que a oferta v2 precisa medir.
    if (planChanged || intervalChanged) {
      events.push({
        type: 'plan_selected',
        metadata: { planCode: nextPlanCode, interval: nextInterval, amountCents },
      });
    }
    if (screensChanged) {
    ...
    const updated = await prisma.checkoutSession.update({
      ...
      data: {
        ...
        marketingOptIn: input.marketingOptIn ?? session.marketingOptIn,
        status,
        identifiedAt,
        lastSeenAt: now,
  ...
  async submitSession(
    ...
    const amountCents = await this.computeAmountCents(
      session.planCode,
      session.screens,
      normalizeInterval(session.billingInterval)
    );
  ...
  async getMetrics(range: { startDate?: Date; endDate?: Date } = {}): Promise<CheckoutMetrics> {
    ...
    const amounts = {
      ...
      paidCashCents: 0,
      abandonedCents: 0,
      abandonedCashCents: 0,
      pendingCents: 0,
      pendingCashCents: 0,
    };
    const paidByInterval: CheckoutMetrics['paidByInterval'] = {
      monthly: { sessions: 0, monthlyEquivalentCents: 0, cashCents: 0 },
      yearly: { sessions: 0, monthlyEquivalentCents: 0, cashCents: 0 },
    };
    ...
    for (const row of rows) {
      ...
      // `amountCents` da sessão é sempre MENSAL EQUIVALENTE; o caixa sai daqui.
      const interval = normalizeInterval(row.billingInterval);
      const cashCents = row.amountCents * (interval === 'yearly' ? 12 : 1);

      if (row.status === 'paid') {
        amounts.paidCents += row.amountCents;
        amounts.paidCashCents += cashCents;
        paidByInterval[interval].sessions += 1;
        paidByInterval[interval].monthlyEquivalentCents += row.amountCents;
        paidByInterval[interval].cashCents += cashCents;
      }
      if (row.status === 'payment_pending') {
        amounts.pendingCents += row.amountCents;
        amounts.pendingCashCents += cashCents;
      }
      if (row.status === 'abandoned') {
        amounts.abandonedCents += row.amountCents;
        amounts.abandonedCashCents += cashCents;
